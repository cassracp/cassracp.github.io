 ...
 * Patched to work with modern TinyMCE API and modern PHP backend.
 */
(function() {
    'use strict';
    ...
    tinymce.PluginManager.add('nanospell', function(editor, url) {
        ...
        var service_url = '/api/spellcheck.php';
        ...
        function checkNow() {
            ...
            // *** CORREÇÃO DO PAYLOAD ***
            // Enviando o JSON no formato que o nosso PHP espera.
            var message = {
                'lang': nanospell_dictionary,
                'words': words
            };
            ...
            request.onload = function() {
                if (request.status >= 200 && request.status < 400) {
                    try {
                        ...
                        // A resposta do nosso PHP é direta, sem 'result'
                        mark(data.words);
                    } catch (e) {
                         console.error("NanoSpell: Failed to parse JSON response.", e);
                    }
                }
            };
        ...
        function findAndReplaceDOMText(parentNode, options) {
            var F = options.find, R = options.replace, nodes = textNodes(parentNode), match, find, k, findlen;
            find = F;
            findlen = find.length;
            for (var i = 0; i < nodes.length; i++) {
        ...
        function unmark() {
            ...
            for (var i = 0; i < typos.length; i++) {
                ...
                if (parent) {
                    while (typos[i].firstChild) {
                        parent.insertBefore(typos[i].firstChild, typos[i]);
                    }
        ...
        function suggest(word) {
             // A função de sugestão não foi implementada no lado do servidor PHP simples.
             // Retornamos uma promessa vazia para evitar erros.
        ...
        editor.on('keydown keypress', function(e) {
            ...
            var target = editor.selection.getNode();
            var ch8r = e.keyCode;
            if ((ch8r >= 16 && ch8r <= 31) || (ch8r >= 37 && ch8r <= 40)) { return; }
            if (target.className === "nanospell-typo") {
                target.className = 'nanospell-typo-disabled';
            }
            triggerSpelling(spell_fast_after_spacebar && (ch8r === 32 || ch8r === 10 || ch8r === 13));
        ...
        editor.ui.registry.addToggleButton('nanospell', {
            ...
            onSetup: function(api) {
                ...
                api.setActive(started); // Seta o estado inicial do botão
                return function() {
        ...
        editor.ui.registry.addMenuItem('nanospell', {
            ...
            onSetup: function(api) {
                ...
                editor.on('SpellcheckStart SpellcheckEnd', onSpellcheckToggle);
                api.setActive(started); // Seta o estado inicial do menu
                return function() {
                    editor.off('SpellcheckStart SpellcheckEnd', onSpellcheckToggle);
                };
            }
