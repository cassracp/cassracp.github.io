...
export default async function handler(request, response) {
  ...
  const GITHUB_OWNER = "demariasoftware";
  ...
  if (!path) {
    return response
      .status(400)
      .json({ error: "O caminho do arquivo é obrigatório." });
  }
  ...
  if (!token) {
    return response
      .status(500)
      .json({ error: "Token de acesso não configurado." });
  }
  ...
  try {
    ...
    const detectedEncoding =
      chardet.detect(contentBuffer, { returnAllMatches: false }) || "UTF-8";

    // ISO-8859-1 é frequentemente como o Win-1252 é detectado.
    // Para o TextDecoder do navegador, 'windows-1252' é mais robusto.
    const finalEncoding = detectedEncoding.toUpperCase().includes("8859")
      ? "windows-1252"
      : detectedEncoding;
    ...
    return response
      .status(500)
      .json({ error: "Erro ao buscar conteúdo do arquivo." });
  }
}
